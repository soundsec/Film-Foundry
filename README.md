...
Its models are intentionally approximate: physics-inspired, chemistry-inspired,
and real-workflow-inspired rather than physically exact. The project favors
explainable controls, inspectable intermediate states, and creative material
behavior over laboratory-accurate simulation.

Longer-term material/process/render design notes are tracked in
`docs/MEDIA_PIPELINE_ROADMAP.md`.
...
GUI 默认使用简易模式：常用流程只需要选择胶片材料、冲洗流程和扫描预设，再调少量暗房语言参数。勾选 **专家模式** 后，才会展开显影液类型、定影/清除、药水疲劳、残银、滤色和高光偏色等更细的控制。
...
python run_foundry_cli.py develop input_images outputs/negatives --film-preset clear_modern_negative --develop-preset monobath_clean --save-scanner-raw --layer-pack
...
主 GUI 只负责调用 preset；材料、冲洗流程、扫描解释的制造和保存由独立编辑器完成。
独立运行 `run_film_material_editor.py` 可以编辑胶片材料本体，并把自定义材料保存到 `user_presets/film/`；主 GUI 和 CLI 会优先读取这里的同名 film preset。
独立运行 `run_develop_process_editor.py` 可以编辑冲洗流程/药水流程，并把自定义流程保存到 `user_presets/develop/`。
独立运行 `run_scanner_render_editor.py` 可以编辑扫描/输出解释，并把自定义扫描预设保存到 `user_presets/scanner/`。
...
```bash
python -m compileall half_frame_darkroom run_darkroom.py run_darkroom_gui.py run_film_foundry_launcher.py run_film_material_editor.py run_develop_process_editor.py run_scanner_render_editor.py
```
